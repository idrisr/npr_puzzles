...
def load_json_from_file(p):
    """Takes relative file path of json file and returns list of parsed json"""
    f = open(p, 'r')
    l = json.load(f)
    return l

def load_from_text(p):
    """takes the relative path 'p' of a file and stores each line in a list"""
    f = open(p, 'r')
    l = list()
    for line in f:
...
def dump_dict_to_json(d, p='dump.json'):
    """takes a dictionary object 'd' and dumps its contents to json in file with path 'p'"""
    f = open(p, 'w')
    json.dump(d, f)
